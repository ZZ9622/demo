...
model = AutoModel.from_pretrained(
    ...
    # 【关键1】使用 4bit 量化，把 20GB 压缩到 ~6GB，完美适配你的 RTX 5060
    # load_in_4bit=True,
    # 【关键2】显式指定 bfloat16，这是 RTX 50 系列的原生语言，比 float16 更稳
    torch_dtype=torch.bfloat16,
    # 【关键3】自动分配显存
    device_map="auto",
    # 【关键4】强制使用普通 Attention，避开 "no kernel image" 的兼容性报错
...
video_path = "data/demo-data/def2_cam_00.mp4"
num_segments=64


with torch.no_grad():
  
  pixel_values, num_patches_list, timestamps = load_video(video_path, num_segments=num_segments, max_num=4, get_frame_by_duration=False)
#   pixel_values = pixel_values.to(torch.bfloat16).to(model.device) 
  pixel_values = pixel_values.to(torch.bfloat16).cuda()
#   video_prefix = "".join([f"Frame{i+1}: <image>\n" for i in range(len(num_patches_list))])
  video_prefix = "".join([f"Time {t:.1f}s: <image>\n" for t in timestamps])
